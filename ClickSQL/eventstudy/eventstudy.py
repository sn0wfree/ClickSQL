...
class EventStudy(object):
    @classmethod
    def cal_abnormal_return_all(cls, return_df,
                                event_dict: dict,
                                date='Date',
                                rf='RF', factors=['Mkt_RF'],
                                formula="{stock} ~ {rf} + {factors_str}",
                                event_info: tuple = (250, 20, 10, 1), add_prefix=True, boost=True):
        func = partial(cls.cal_abnormal_return_single,
                       date=date,
        ...
        if boost:
            ...
            h = boost_up(func, tasks, star=True)
        else:
            h = []
            for stock, event_happen_day in event_dict.items():
                event_df = func(return_df, event_happen_day, stock_return=stock)
                h.append(event_df)
        return h

    @staticmethod
    def cal_abnormal_return_single(return_df: pd.DataFrame, event_happen_day: str, stock_return='AMZN', date='Date',
                                   rf='RF', factors=['Mkt_RF'],
                                   formula="{stock} ~ {rf} + {factors_str}",
        ...
        if add_prefix:
            event_df[f'{stock}_expected_return'] = np.dot(event_df[factors].values, beta_Mkt) + alpha
            event_df[f'{stock}_abnormal_return'] = event_df.eval(f'{stock} - {stock}_expected_return')
        #             resistd = event_df[f'{stock}_abnormal_return'].std() / corp_count
        else:
            event_df['expected_return'] = np.dot(event_df[factors].values, beta_Mkt) + alpha
            event_df['abnormal_return'] = event_df.eval(f'{stock} - expected_return')
        #             resistd = event_df[f'{stock}_abnormal_return'].std() / corp_count
